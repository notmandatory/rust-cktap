...
final class CardEmulator: CkTransport {
    ...
    let STATUS_COMMAND: [UInt8] = [0xA1, 0x63, 0x63, 0x6d, 0x64, 0x66, 0x73, 0x74, 0x61, 0x74, 0x75, 0x73]
    ...
    func transmitApdu(commandApdu: Data) async throws -> Data {
            ...
            guard socketFd != -1 else {
                throw CkTapError.Transport(msg: "Failed to create socket: \(String(cString: strerror(errno)))")
            }
            defer {
                close(socketFd)
            }
            ...
            guard connectResult != -1 else {
                throw CkTapError.Transport(msg: "Failed to connect to socket: \(String(cString: strerror(errno)))")
            }
            ...
            guard writeResult != -1 else {
                throw CkTapError.Transport(msg: "Failed to write to socket: \(String(cString: strerror(errno)))")
            }
            
            guard writeResult == commandData.count else {
                throw CkTapError.Transport(msg: "Incomplete write to socket: wrote \(writeResult) of \(commandData.count) bytes")
            }
            ...
            guard readResult != -1 else {
                throw CkTapError.Transport(msg: "Failed to read from socket: \(String(cString: strerror(errno)))")
            }
