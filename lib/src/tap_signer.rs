...
pub trait TapSignerShared<T: CkTransport>: Authentication<T>
where
    T: CkTransport,
{
    ...
    fn init(
        ...
        chain_code: [u8; 32],
        cvc: &str,
    ) -> impl Future<Output = Result<NewResponse, TapSignerError>> {
        async move {
    ...
    fn status(&mut self) -> impl Future<Output = Result<StatusResponse, Error>> {
        async {
    ...
    fn sign(
        ...
        sub_path: Vec<u32>,
        cvc: &str,
    ) -> impl Future<Output = Result<SignResponse, Error>> {
        async move {
    ...
    fn sign_psbt(
        ...
        cvc: &str,
    ) -> impl Future<Output = Result<bitcoin::Psbt, PsbtSignError>> {
        use bitcoin::{
            secp256k1::ecdsa,
    ...
    fn derive(
        ...
        path: &[u32],
        cvc: &str,
    ) -> impl Future<Output = Result<DeriveResponse, TapSignerError>> {
        async {
    ...
    fn change(
        ...
        new_cvc: &str,
        cvc: &str,
    ) -> impl Future<Output = Result<ChangeResponse, TapSignerError>> {
        async move {
            if new_cvc.len() < 6 {
...
impl<T: CkTransport> TapSignerShared<T> for TapSigner<T> {}
